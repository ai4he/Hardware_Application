...
def run(args):
    ...
    for frame_idx, batch in enumerate(predictor.dataset):
        ...
        for i in range(n):
            ...
            # assuming classes are available in the output along with bounding boxes
            for det in dets.cpu().detach().numpy():
                # Extract the bounding box and class
                bbox, cls = det[:4], det[-1]


                x, y, x2, y2 = map(int, bbox)

                if cls == 'window':  # or any condition that identifies your class of interest
                    ...
                    for pt in zip(*loc[::-1]):
                        ...
                        cv2.waitKey(50000)
                        cv2.imwrite(os.path.join(os.getcwd(), f'result_{frame_number}.jpg'), im0)
