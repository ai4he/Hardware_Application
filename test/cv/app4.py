...
#                     # Initialize a new tracker for the detected window
#                     tracker = cv2.TrackerKCF_create()
#                     tracker.init(frame, bbox)

#                     # Assign a new unique ID to the tracker
#                     tracker_id = len(tracker_dict) + 1
#                     #tracker_dict[tracker_id] = {"tracker": tracker, "status": "active"}
#                     tracker_dict[tracker_id] = {"tracker": tracker, "status": "active", "previous_status": "inactive"}


#                     # Insert new record into the database
#                     title_bar = frame[10:50, 50: 1920]
#                     window_title = ocr_image(title_bar)
#                     wa.insert_window_activity(tracker_id, "active", window_title)

#         frames_since_last_detection = 0
#     else:
#         # Update existing trackers by creating a copy of the keys
#         for tracker_id in list(tracker_dict.keys()):
#             tracker_data = tracker_dict[tracker_id]
#             tracker = tracker_data["tracker"]
#             success, bbox = tracker.update(frame)
#             if success:
#                 tracker_data["status"] = "active"
#                 x, y, w, h = map(int, bbox)
#                 cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)
#                 cv2.putText(frame, f"ID: {tracker_id} {model.names[class_type]}", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
                
    
#                 # Check if the status has changed from "inactive" to "active"
#                 if tracker_data["status"] == "inactive":
#                     tracker_data["status"] = "active"
#                     title_bar = frame[10:50, 50: 1920]
#                     window_title = ocr_image(title_bar)
#                     wa.insert_window_activity(tracker_id, "active", window_title)  # Insert new record
#             else:
#                 # If the tracker fails, remove it from the dictionary
#                 tracker_dict.pop(tracker_id)

#     frames_since_last_detection += 1

#     cv2.imshow('Screen Capture', frame)
#     #screen_shot = show_image(frame)
#     #title_bar = frame(0, 0, 1920, 30))
